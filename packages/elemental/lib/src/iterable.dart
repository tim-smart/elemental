...
/// {@template elemental_iterable_extension_head}
...
extension ElementalOnIterable<T> on Iterable<T> {
  /// {@macro elemental_iterable_extension_head}
  ///
  /// Same as `firstOption`.
  Option<T> get head {
    var it = iterator;
    if (it.moveNext()) return Option.of(it.current);
    return const None();
  }
  ...
  Option<T> get lastOption => isEmpty ? const None() : Option.of(last);
  ...
  Option<Iterable<T>> get tail => isEmpty ? const None() : Option.of(skip(1));
  ...
  Option<Iterable<T>> get init {
    ...
    return Option.of(this.dropRight(1));
  }
...
/// Functional programming functions on `Iterable<Iterable<T>>` using `elemental`.
extension ElementalOnIterableOfIterable<T> on Iterable<Iterable<T>> {
